...
$cell = $simpla->request->post('cell');
$cellval = $simpla->request->post('cellval');
...
switch ($object)
{
    ...
    case 'user':
        ...
        $result = $simpla->users->add_uprice($values);
        break;
    case 'userinfo':
        if($simpla->managers->access('users'))
    ...
    case 'useradd':
        if($simpla->managers->access('users'))
        $result = $simpla->users->add_user($values);
        break;
    case 'add_user_basket':
        if($simpla->managers->access('users'))
        $result = $simpla->users->add_user_basket($values);
        break;
    case 'doc':
        if($simpla->managers->access('users'))
        $result = $simpla->users->update_user_doc($id, $values);
        break;
    case 'doc_new':
        if($simpla->managers->access('users'))
        $result = $simpla->users->add_user_doc($values);
        break;
    case 'change_supplys_status':
        if($simpla->managers->access('users'))
        $result = $simpla->users->update_uprice($id, $values);
        break;
    case 'change_isupplier':
        if($simpla->managers->access('users'))
        $result = $simpla->users->update_uprice($id, $values);
        break;
    case 'add_supplier':
    ...
    case 'update_product_supplier':
        if($simpla->managers->access('products'))
        $result = $simpla->products->update_product($id, array($cell => $cellval));
        break;
