...
class DocsAdmin extends Simpla
{
	function fetch()
	{
		...
		$doc_id = $this->request->get('doc_id', 'integer');
		$doc = $this->users->get_user_doc($doc_id);

		$this->design->assign('doc', $doc);

		//$this->design->assign('json_doc', json_encode($users));

		foreach ($this->users->get_user_docs('ruslan@rosstok.ru') as $d)
			$docs[$d->id] = $d;
		...
		return $this->body = $this->design->fetch('docs.tpl');
	}
}