...
//$variant->price = $xml_variant->Цены->Цена->ЦенаЗаЕдиницу;	//отключено обновление цены

	//$simpla->db->query("SELECT v.price FROM __variants v WHERE v.id=?", $variant_id);
	//$variant_price = $simpla->db->result('price');
	//$variant->price = $variant_price;

	//$simpla->db->query('SELECT v.id FROM __variants v WHERE v.external_id=? AND product_id=(SELECT p.id FROM __products p WHERE p.external_id=? LIMIT 1)', $variant_1c_id, $product_1c_id);
	//$variant_id = $simpla->db->result('price');

	//$variant->price = 777;

	$variant->zakupka = $xml_variant->Цены->Цена->ЦенаЗаЕдиницу;
	...
	if(!empty($xml_variant->Цены->Цена->Валюта))
	{
		...
		{
			$variant->zakupka = floatval($variant->zakupka)*$variant_currency->rate_to/$variant_currency->rate_from;
		}
	}
	...
	if(empty($variant_id))
		...
		$simpla->variants->update_variant($variant_id, $variant);
	    //$simpla->variants->add_variant(array('productid'=>$variant->productid));
}
