	...
	<input type="hidden" name="session_id" value="{$smarty.session.id}">
		...
		<div id="list">
			...
			<div class="{if $order->paid}green{/if} row">
				...
				<div class="order_date cell">
				Покупатель:<br> {$order->name|escape}
				</div>
				<div class="order_date cell">
	 				Заявка от:<br> {$order->date|date} в {$order->date|time}
				</div>
				<div class="order_date cell">
	 				Дата отгрузки: {$order->dataotgruzki}
				</div>
				<div class="order_date cell">
	 				Сумма:<br> {$order->total_price|escape} {$currency->sign}
				</div>
