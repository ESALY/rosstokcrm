...
<h1 class="ie6_main_h1">{if $cart->purchases}В корзине {$cart->total_products} {$cart->total_products|plural:'товар':'товаров':'товара'}
{else}Корзина пуста{/if}<i></i></h1>
...
<tr>
	...
	<td class="image">
		...
		{if $image}
		<a href="products/{$purchase->product->url}"><img src="{$image->filename|resize:40:40}" alt="{$product->name|escape}"></a>
		{/if}
	</td>
...
<h1 class="ie6_main_h1">Оформление заказа
<i></i></h1>

<div class="basket-tabs">
    <div class="section">
        <ul class="tabs">
            <li class="current">Шаг №1. Выберите способ доставки</li>
            <li>Шаг №2. Заполните необходимую информацию</li>
        </ul>
...
<div class="form cart_form">         
 ...
 <table width="100%" border="1">
  ...
  <tr>
    <td colspan="2"><label>Комментарий к&nbsp;заказу</label>
	<textarea name="comment" id="order_comment">{$comment|escape}</textarea></td>
    </tr>
  <tr>
    <td><label>Введите код с картинки</label><div class="captcha"><img src="captcha/image.php?{math equation='rand(10,10000)'}" alt='captcha'/></div> 
	<input class="input_captcha" id="comment_captcha" type="text" name="captcha_code" value="" data-format="\d\d\d\d" data-notice="Введите капчу"/></td>
    <td align="right"><input type="submit" name="checkout" class="button" value="Оформить заказ">
	</td>
  </tr>
</table>
