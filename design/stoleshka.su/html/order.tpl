...
<h1 class="ie6_main_h1">Спасибо! Ваш заказ №{$order->id} 
{if $order->status == 0}принят{/if}
{if $order->status == 1}в обработке{elseif $order->status == 2}выполнен{/if}
{if $order->paid == 1}, оплачен{else}{/if}, в течении часа с Вами свяжется менеджер отдела сбыта.<i></i></h1>
...
<tr>
	...
	<th class="amount"></th>
	<th class="price">
		Итоговая сумма {$order->total_price|convert}&nbsp;{$currency->sign}
	</th>
</tr>
