...
function RSPQuickbuyOnWindowResize()
{
	var constSiteW1 = [800,1050,1300];
	var bodyW = $('body').width();
	var siteW = $('.wrap_inner').width();
	var brands_block = $('.brands_list_container').length;
	if(siteW==constSiteW1[0])
	{
		if(brands_block>0)
		{
			$('.quickbuy').find('.quickbuy-item:eq(1)').hide();
			$('.quickbuy').find('.quickbuy-item:eq(2)').hide();
			$('.quickbuy').find('.quickbuy-item:eq(3)').hide();
		} else {
			$('.quickbuy').find('.quickbuy-item:eq(1)').show();
			$('.quickbuy').find('.quickbuy-item:eq(2)').hide();
			$('.quickbuy').find('.quickbuy-item:eq(3)').hide();
		}
	} else if(siteW==constSiteW1[1])
	{
		if(brands_block>0)
		{
			$('.quickbuy').find('.quickbuy-item:eq(1)').show();
			...
			$('.quickbuy').find('.quickbuy-item:eq(1)').show();
			$('.quickbuy').find('.quickbuy-item:eq(2)').show();
			$('.quickbuy').find('.quickbuy-item:eq(3)').hide();
		}
	} else {
		if(brands_block>0)
		{
			...
			$('.quickbuy').find('.quickbuy-item:eq(1)').show();
			$('.quickbuy').find('.quickbuy-item:eq(2)').show();
			$('.quickbuy').find('.quickbuy-item:eq(3)').show();
		}
	}
}
...
$(document).ready(function(){
	...
	RSPQuickbuyOnWindowResize();
	$(window).resize(function(){
		RSPQuickbuyOnWindowResize();
	});
	
	/* box_shadow on Hover */
	$('.quickbuy').find('.quickbuy-item').on('mouseenter',function(){
