			...
			g.addClass("d_i-b hidePart"), g.is("[data-trigger]") || (h.html(h.data("show")), g.off("click.showhidepart").on("click.showhidepart", function() {
				...
				if ("no" !== d.data("show") && d.data("show")) {
					...
					d.parent().nextAll(".wrapper-h").animate({
						...
						$(this).css("max-height", "none").removeClass("full-height").addClass("cut-height"), e.hide().html(e.data("show")).fadeIn(c)
					}), d.data("show", "no")
				} else {
...
function init() {
	...
	var b = new itemUserToolbar,
		c = $(".btn-to-up");
	c.click(function() {
		$("html, body").animate({
			scrollTop: "0"
		})
	}), b.show($(".items-user-toolbar"), $(".btn-toggle-toolbar > button"), ".box-1, .box-2, .box-3", c), b.resize($(".frame-user-toolbar"), c), $.existsN($(".animateListItems.table")) && decorElemntItemProduct();
	var d = $(".frame-label").length;
	...
	$("#sort").on("change.orderproducts", function() {
		e.find("input[name=order]").val($(this).val()), e.submit()
	}), $("#sort2").on("change.countvisibleproducts", function() {
		e.find("input[name=user_per_page]").val($(this).val()), e.submit()
	...
	}), a.on("remove.Cart", function(a) {
		ShopFront.Cart.processBtnBuyCount(a.id, "remove", a.kit)
	}), a.on("remove.Cart сhange.Cart", function(a) {
	...
	}), a.on("compare_list_add compare_list_rm compare_list_sync", function() {
		ShopFront.CompareList.count(), ShopFront.CompareList.process(), decorElemntItemProduct()
	}), a.on("wish_list_sync", function() {
		global.processWish(), global.wishListCount()
	}), a.on("widget_ajax", function(a) {
	...
	}), a.on("lazy.after", function(a) {
		a.el.addClass("load")
	}), a.on("tabs.beforeload", function(a) {
		a.els.filter(".active").append('<div class="preloader"></div>')
	}), a.on("tabs.afterload", function(a) {
		ShopFront.Cart.pasteItems(a.el), a.els.find(preloader).remove()
	}), a.on("autocomplete.fewLength", function(a) {
		a.el.tooltip("show", {
	...
	a.on("rendercomment.after", function(a) {
		showHidePart(a.el.find(".product-comment")), a.el.find("[data-drop]").drop(optionsDrop), a.el.find(preloader).remove()
	}), a.on("autocomplete.after rendercomment.after imageapi.pastemsg showCleaverFilter tabs.afterload renderorder.after", function(a) {
		a.el.parent().is(":visible") && drawIcons(a.el.parent().find(selIcons))
	}), a.on("imageapi.pastemsg imageapi.hidemsg", function(a) {
		a.el.closest("[data-elrun]").drop("limitSize").drop("heightContent").drop("center")
	}), a.on("imageapi.before_refresh_reload", function(a) {
	...
	}), a.on("autocomplete.before showActivity before_add_to_compare before_delete_compare discount.load_certificate beforeAdd.Cart", function() {
		$.fancybox.showActivity()
	}), a.on("autocomplete.after after.drop closed.drop hideActivity compare_list_add compare_list_rm compare_list_sync imageapi.success getTpl.Cart", function() {
		$.fancybox.hideActivity()
	}), a.on("comments.showformreply tabs.showtabs after.drop", function(a) {
		ltie7 && ieBoxSize(a.el.find(':input:not(button):not([type="button"]):not([type="reset"]):not([type="submit"])'))
	}), a.on("comments.beforeshowformreply", function(a) {
	...
	}), a.on("comments.beforehideformreply", function(a) {
		a.el.closest(".patch-product-view").removeAttr("style")
	}), a.on("menu.showDrop", function() {
	...
	var b = {
		init: function(b) {
			...
			j && s.on("click.autocomplete", function(b) {
				...
				var h = b.keyCode;
				g > r ? (d.tooltip("remove"), 27 !== h && 40 !== h && 38 !== h && 39 !== h && 37 !== h && 13 !== h && 0 !== g && "" !== a.trim(d.val()) ? f = setTimeout(c, m) : 0 === g && e()) : 13 !== h || j ? a(document).trigger({
					type: "autocomplete.fewLength",
					el: d,
					value: r
				}) : s.closest("form").submit(), p.val().length <= r && j ? s.off("click.autocomplete").on("click.autocomplete", function(b) {
					b.preventDefault(), p.focus(), a(document).trigger({
						type: "autocomplete.fewLength",
						el: p,
						value: r
			...
			return this.each(function() {
				...
				if (a.existsN(e)) {
					...
					Q || k.add(k.find(".helper:first")).css("height", ""), k.each(function(c) {
						...
						d.data("kk", 0)
					}), Q || k.css("height", f), Q || k.find(".helper:first").css("height", f), e.removeClass(P);
					var U = "";
					"toggle" === H && (H = "click"), "toggle" === I && (I = "click"), k.off("click").off("hover")[H](function(b) {
						...
						if (!a) var a = window.event;
						27 === a.keyCode && d()
					}), n.find("a").off("click.menuref").on("click.menuref", function(b) {
						return "click" === I ? a.existsN(a(this).next()) && A ? (b.preventDefault(), !0) : (b.stopPropagation(), !0) : void b.stopPropagation()
					}), k.find("a:first").off("click.menuref").on("click.menuref", function(b) {
			...
			return void 0 === a && (a = this.self ? this.self : this), a.each(function() {
				...
				if (b.limitContentSize) {
					...
					if (c || (a.show(), d && d.show()), b.dropContent) {
						...
						if (e.data("jsp") && e.data("jsp").destroy(), e = a.find($(b.dropContent)).filter(":visible").css({
							height: ""
						}), $.existsN(e)) {
							var f = b.elrun,
								g = !1,
							...
							if (b.scrollContent) try {
								g = e.jScrollPane(scrollPane).data("jsp"), $.existsN(e.find(".jspPane")) && (h = e.find(".jspPane").outerHeight())
							} catch (i) {
								e.css("overflow", "auto")
							}
							var j = a.outerHeight(),
								k = a.height(),
								l = a.find($(b.dropHeader)).outerHeight() + a.find($(b.dropFooter)).outerHeight();
							if ("noinherit" === b.place) {
								var m = 0,
									n = b.placement;
								"object" == typeof n ? (void 0 !== n.top && (m = wnd.height() - n.top - l - (j - k)), void 0 !== n.bottom && (m = n.bottom - l - (j - k))) : (n.search(/top/) >= 0 && (m = wnd.height() - f.offset().top - l - f.outerHeight() - (j - k)), n.search(/bottom/) >= 0 && (m = f.offset().top - l - (j - k))), m > h ? e.css("height", h) : e.css("height", m)
							} else h + l > k ? e.css("height", k - l) : e.css("height", h);
							g && g.reinitialise()
						}
					}
					c || (a.hide(), d && d.hide())
...
"col" === typeMenu && (optionsMenu.countColumn = 5), "row" === typeMenu && (optionsMenu.sub2Frame = ".frame-l2", optionsMenu.dropWidth = 475);
var scrollPane = {
		animateScroll: !0,
		showArrows: !0,
...
var ShopFront = {
		...
		CompareList: {
			process: function() {
				...
				$(".btnCompare " + genObj.textEl).off("click.inCompare"), $("." + genObj.toCompare).each(function() {
					...
					/category|product/.test(c) ? (-1 !== c.indexOf("category") ? b = c.substr(c.lastIndexOf("/") + 1, c.length) : -1 !== c.indexOf("product") && (b = hrefCategoryProduct.substr(hrefCategoryProduct.lastIndexOf("/") + 1, hrefCategoryProduct.length)), document.location.href = locale + "/shop/compare#tab_" + b) : document.location.href = locale + "/shop/compare"
				})
			},
			count: function() {
				var a = Shop.CompareList.all().length,
					b = $(genObj.tinyCompareList).find("[data-href]").drop("destroy").off("click.tocompare");
				a > 0 ? ($(genObj.tinyCompareList).addClass(genObj.isAvail).find(genObj.blockNoEmpty).show().end().find(genObj.blockEmpty).hide(), b.on("click.tocompare", function() {
					location.href = $(this).data("href")
				})) : ($(genObj.tinyCompareList).removeClass(genObj.isAvail).find(genObj.blockNoEmpty).hide().end().find(genObj.blockEmpty).show(), b.drop()), $(genObj.countTinyCompareList).each(function() {
					$(this).html(a)
	...
	global = {
		...
		wishListCount: function() {
			...
			a > 0 ? ($(genObj.tinyWishList).addClass(genObj.isAvail).find(genObj.blockNoEmpty).show().end().find(genObj.blockEmpty).hide(), b.on("click.towish", function() {
				location.href = $(this).data("href")
			})) : ($(genObj.tinyWishList).removeClass(genObj.isAvail).find(genObj.blockNoEmpty).hide().end().find(genObj.blockEmpty).show(), b.drop()), $(genObj.countTinyWishList).each(function() {
				$(this).html(a)
