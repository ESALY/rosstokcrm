...
class Variants extends Simpla
{
	...
	*/
	public function get_variants($filter = array())
	{
		...
		$query = $this->db->placehold("SELECT v.id, v.akcija_procent, v.akcija_zakupka, v.spec_zakupka, v.akcija_kolichestvo, v.akcija_cena, v.spec_procent, v.spec_kolichestvo, v.spec_cena, v.product_id, v.price, v.zakupka, v.kopt, v.opt, v.roznper, v.optper, v.koptper, v.lastedit, NULLIF(v.compare_price, 0) as compare_price, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.name, v.attachment, v.position
	...
	public function update_variant_1c($id, $variant)
	{
		$query = $this->db->placehold("UPDATE __variants SET ?% WHERE id=? LIMIT 1", $variant, intval($id));
		$this->db->query($query);
		return $id;
