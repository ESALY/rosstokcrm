...
class Products extends Simpla
{
	...
	*/
	public function get_products($filter = array())
	{
		...
		if(!empty($filter['supplys_prouduct']))
		{
			$category_id_filter = $this->db->placehold('INNER JOIN __products_uprice sp ON sp.product_id = p.id AND sp.category_id in(?@)', (array)$filter['category_id']);
			$group_by = "GROUP BY p.id";
		}
